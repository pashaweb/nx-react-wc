...
interface PicturesAnaliticsState {
  ...
  getPicture: (id: number) => TpictureAnlytics | null;
}

export const usePictureAnaliticsStore = create<PicturesAnaliticsState>(
  (set, get) => ({
    list: initialState,
    selectedPictureId: null,
    selectedShapeId: null,
    selectedPicture: null,
    setSelectedPictureId: (id: number) =>
      set({
    ...
    addPicture: (picture: TpictureAnlytics) =>
      set({ list: [...get().list, picture] }),
    removePicture: (id: number) =>
      set({ list: get().list.filter((picture) => picture.id !== id) }),
    updatePicture: (picture: TpictureAnlytics) =>
      set({ list: get().list.map((p) => (p.id === picture.id ? picture : p)) }),
    ...
    getPicture: (id: number) =>
      get().list.find((picture) => picture.id === id) || null,
  })
);
